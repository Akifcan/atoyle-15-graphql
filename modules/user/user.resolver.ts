...
export const userResolvers = {
  profile: async (props: { slug: string }, context: ContextProps): Promise<UserProfile> => {
    const { slug } = props

    authGuard(context.headers.authorization)

    const query = await Db.client.query('SELECT id, name, department, description, email, slug FROM employee WHERE slug = $1 AND isActive = true', [slug])
  ...
  signIn: async (props: { email: String; password: String }): Promise<User> => {
    ...
    const query = await Db.client.query('SELECT * FROM employee where email = $1 AND password = $2 AND isActive = true', [email, password])
